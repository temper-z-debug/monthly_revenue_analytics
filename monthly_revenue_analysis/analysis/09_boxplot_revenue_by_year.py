...
def main():
    ...
    # ---- Identify outlier months for 2011 using IQR ----
    outliers_2011 = None
    if 2011 in years:
    ...
    whiskers = boxplot["whiskers"]  # 2 per box: lower, upper
    ...
    # Expand x-limits to make room for external labels
    x_min = 0.5
    x_max = len(years) + 0.5
    ...
    # ---- n= labels (above each upper whisker) ----
    ...
    # ---- median/mean labels (auto left/right, readable) ----
    bbox_style = dict(
    ...
    for i in range(len(years)):
        ...
        # small vertical separation if too close
    ...
    # ---- Annotate 2011 outlier months (aligned to outlier points, show YYYY-MM only) ----
    # ---- Annotate 2011 outlier months (arrow, not touching data point) ----
    if outliers_2011 is not None and not outliers_2011.empty:
        ...
        for k, (_, row) in enumerate(outliers_2011.iterrows()):
            label = f"{int(row['year'])}-{int(row['month']):02d}"  # 只显示 YYYY-MM
